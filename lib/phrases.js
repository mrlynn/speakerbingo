export const PHRASE_CATEGORIES = {
  'sunrise-regulars': {
    ...
    phrases: [
      ...
      'sick as your secrets', 'dont drink and go to meetings', 'never had it so good', 'HALT: Hungry, Angry, Lonely, Tired',
      'Your best thinking got you here', 'This too shall pass', 'One day at a time', 'Easy does it', 'Let go and let God',
      'Progress not perfection', 'Turn it over', 'Let go and let God', 'HALT: Hungry, Angry, Lonely, Tired',
      'This too shall pass', 'Rigorous honesty', 'Willing to go to any length', 'Constitutionally incapable',
      'Hopeless state of mind and body', 'Spiritual malady', 'Bedevilments', 'Irritable, restless, and discontented',
      'Pitiful and incomprehensible demoralization', 'Mental obsession', 'Physical allergy', 'Rule 62', 'Never take yourself too seriously',
      'Keep an open mind', 'Surrender to win', 'Fake it till you make it', 'Stick with the winners', 'Take what you like and leave the rest',
      'Principles before personalities', 'When all else fails, follow directions', 'Meeting makers make it', 'Call your sponsor', 'Read the book',
      ...
      'Slip vs relapse', 'Bottom line', 'Rock bottom', 'Gift of desperation', 'I have reflex.', 'anyway', 'I have a disease.', '...kind of thing', 'its a good day to have a good day'
    ]
  },
...
export function getRandomPhrasesFromCategory(categoryKey, count = 24) {
  ...
  const uniquePhrases = [...new Set(category.phrases)]
  
  // If we need more phrases than available, return all unique phrases
  if (count >= uniquePhrases.length) {
    return shuffleArray(uniquePhrases)
  }
  
  // Use improved randomization with better entropy
  const shuffled = enhancedShuffle(uniquePhrases)
  return shuffled.slice(0, count)
}

/**
 * Enhanced shuffle algorithm with better entropy and distribution
 * Uses Fisher-Yates shuffle with improved random selection and multiple passes
 */
function enhancedShuffle(arr) {
  const shuffled = [...arr]
  ...
  const getRandom = typeof crypto !== 'undefined' && crypto.getRandomValues
    ? () => {
  ...
  // Multiple shuffle passes for better distribution
  for (let pass = 0; pass < 2; pass++) {
    // Fisher-Yates shuffle with improved random selection
...
/**
 * Fallback simple shuffle (Fisher-Yates) for compatibility
 */
function shuffleArray(arr) {
  return enhancedShuffle(arr)
}